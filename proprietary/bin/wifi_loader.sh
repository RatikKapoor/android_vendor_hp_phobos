...
#broadcomm comm chip
	...
	if [ $brcm_symlink = y ]; then
		  /system/bin/log -t "wifiloader" -p i  "BCM4334 chip identified ---- brcm_symlink = y"
		...
		if [ $chip = "43341" ]; then
		  ...
		  /system/bin/log -t "wifiloader" -p i  "BCM4334 chip identified ---- brcm_symlink = y, nvram !!"
#     /system/bin/ln -s /system/etc/nvram_$chip.txt /data/misc/wifi/firmware/nvram.txt
			/system/bin/ln -s /system/etc/nvram_4334_ww.txt /data/misc/wifi/firmware/nvram_ww.txt
			/system/bin/ln -s /system/etc/nvram_4334_us.txt /data/misc/wifi/firmware/nvram_us.txt
			/system/bin/ln -s /system/etc/nvram_4334_id.txt /data/misc/wifi/firmware/nvram_id.txt
		fi
	fi
	...
	if [  $chip = "4335" ]; then
		...
		setprop wifi.driver_param_path "/sys/module/bcm43241/parameters/firmware_path"
	else
		insmod /system/lib/modules/bcmdhd.ko
		setprop wifi.driver_param_path "/sys/module/bcmdhd/parameters/firmware_path"
	fi
